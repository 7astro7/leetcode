...
"""
You are given the logs for users' actions on LeetCode, and an 
integer k. The logs are represented by a 2D integer array logs where 
each logs[i] = [IDi, timei] indicates that the user with IDi 
...
for each j (1 <= j <= k), answer[j] is the number of users whose UAM 
equals j.
    ...
    def uam(
        ...
        for log in logs:
            ...
            if uid not in self.data.keys():
                self.data[uid] = {log[1]}
            elif len(self.data[uid]) <= k:
                self.data[uid].add(log[1])
