...
class Solution:
    ...
    def remove_occurences(self, s: str, part: str) -> str:
        if part not in s:
            return s
        new_str = s
        while True:
            if new_str.find(part) == -1:
                return new_str
            idx_of_part = new_str.find(part)
            new_str = new_str[:idx_of_part] + new_str[idx_of_part + len(part):]
